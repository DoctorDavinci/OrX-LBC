...
namespace OrX.spawn
{
    ...
    public class SpawnOrX_SMI : MonoBehaviour
    {
        ////////////////////////////
        ...
        /// /////////////////////////
        /// </summary>
        public static SpawnOrX_SMI instance;
        ...
        private string SMIVesselName = string.Empty;

        private string OrX_SMI01 = string.Empty;
        private string OrX_SMI02 = string.Empty;
        private string OrX_SMI03 = string.Empty;
        private string OrX_SMI04 = string.Empty;
        private string OrX_SMI05 = string.Empty;
        private string OrX_SMI06 = string.Empty;
        private string OrX_SMI07 = string.Empty;
        private string OrX_SMI08 = string.Empty;
        private string OrX_SMI09 = string.Empty;
        private string OrX_SMI10 = string.Empty;
        ...
        private void Awake()
        {
            var _OrX_SMI01 = Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location);
            flagURL = "OrX_icon";
            OrX_SMI01 = _OrX_SMI01 + "\\PluginData" + "\\VesselData" + "\\SportingGoodsDepartment" + "\\SMI" + "\\OrXSMI01.craft";
            Debug.Log("[Spawn OrX Tank] craftURL: " + OrX_SMI01);

            var _OrX_SMI02 = Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location);
            flagURL = "OrX_icon";
            OrX_SMI02 = _OrX_SMI02 + "\\PluginData" + "\\VesselData" + "\\SportingGoodsDepartment" + "\\SMI" + "\\OrXSMI02.craft";
            Debug.Log("[Spawn OrX Tank] craftURL: " + OrX_SMI02);

            var _OrX_SMI03 = Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location);
            flagURL = "OrX_icon";
            OrX_SMI03 = _OrX_SMI03 + "\\PluginData" + "\\VesselData" + "\\SportingGoodsDepartment" + "\\SMI" + "\\OrXSMI03.craft";
            Debug.Log("[Spawn OrX Tank] craftURL: " + OrX_SMI03);

            var _OrX_SMI04 = Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location);
            flagURL = "OrX_icon";
            OrX_SMI04 = _OrX_SMI04 + "\\PluginData" + "\\VesselData" + "\\SportingGoodsDepartment" + "\\SMI" + "\\OrXSMI04.craft";
            Debug.Log("[Spawn OrX Tank] craftURL: " + OrX_SMI04);

            var _OrX_SMI05 = Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location);
            flagURL = "OrX_icon";
            OrX_SMI05 = _OrX_SMI05 + "\\PluginData" + "\\VesselData" + "\\SportingGoodsDepartment" + "\\SMI" + "\\OrXSMI05.craft";
            Debug.Log("[Spawn OrX Tank] craftURL: " + OrX_SMI05);

            var _OrX_SMI06 = Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location);
            flagURL = "OrX_icon";
            OrX_SMI06 = _OrX_SMI06 + "\\PluginData" + "\\VesselData" + "\\SportingGoodsDepartment" + "\\SMI" + "\\OrXSMI06.craft";
            Debug.Log("[Spawn OrX Tank] craftURL: " + OrX_SMI06);

            var _OrX_SMI07 = Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location);
            flagURL = "OrX_icon";
            OrX_SMI07 = _OrX_SMI07 + "\\PluginData" + "\\VesselData" + "\\SportingGoodsDepartment" + "\\SMI" + "\\OrXSMI07.craft";
            Debug.Log("[Spawn OrX Tank] craftURL: " + OrX_SMI07);

            var _OrX_SMI08 = Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location);
            flagURL = "OrX_icon";
            OrX_SMI08 = _OrX_SMI08 + "\\PluginData" + "\\VesselData" + "\\SportingGoodsDepartment" + "\\SMI" + "\\OrXSMI08.craft";
            Debug.Log("[Spawn OrX Tank] craftURL: " + OrX_SMI08);

            var _OrX_SMI09 = Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location);
            flagURL = "OrX_icon";
            OrX_SMI09 = _OrX_SMI09 + "\\PluginData" + "\\VesselData" + "\\SportingGoodsDepartment" + "\\SMI" + "\\OrXSMI09.craft";
            Debug.Log("[Spawn OrX Tank] craftURL: " + OrX_SMI09);

            var _OrX_SMI10 = Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location);
            flagURL = "OrX_icon";
            OrX_SMI10 = _OrX_SMI10 + "\\PluginData" + "\\VesselData" + "\\SportingGoodsDepartment" + "\\SMI" + "\\OrXSMI10.craft";
            Debug.Log("[Spawn OrX Tank] craftURL: " + OrX_SMI10);
        ...
        public void CheckSpawnTimer()
        {
            if (sg01)
            {
                ...
                timer = true;
                SMIVesselName = "";
                StartCoroutine(SpawnCraftRoutine(OrX_SMI01));
            }
            else
            {
                if (sg02)
                {
                    ...
                    timer = true;
                    SMIVesselName = "";
                    StartCoroutine(SpawnCraftRoutine(OrX_SMI02));
                }
                else
                {
                    if (sg03)
                    {
                        ...
                        timer = true;
                        SMIVesselName = "";
                        StartCoroutine(SpawnCraftRoutine(OrX_SMI03));
                    }
                    else
                    {
                        if (sg04)
                        {
                            ...
                            timer = true;
                            SMIVesselName = "";
                            StartCoroutine(SpawnCraftRoutine(OrX_SMI04));
                        }
                        else
                        {
                            if (sg05)
                            {
                                ...
                                timer = true;
                                SMIVesselName = "";
                                StartCoroutine(SpawnCraftRoutine(OrX_SMI05));
                            }
                            else
                            {
                                if (sg06)
                                {
                                    ...
                                    timer = true;
                                    SMIVesselName = "";
                                    StartCoroutine(SpawnCraftRoutine(OrX_SMI06));
                                }
                                else
                                {
                                    if (sg07)
                                    {
                                        ...
                                        timer = true;
                                        SMIVesselName = "";
                                        StartCoroutine(SpawnCraftRoutine(OrX_SMI07));
                                    }
                                    else
                                    {
                                        if (sg08)
                                        {
                                            ...
                                            timer = true;
                                            SMIVesselName = "";
                                            StartCoroutine(SpawnCraftRoutine(OrX_SMI08));
                                        }
                                        else
                                        {
                                            if (sg09)
                                            {
                                                ...
                                                timer = true;
                                                SMIVesselName = "";
                                                StartCoroutine(SpawnCraftRoutine(OrX_SMI09));
                                            }
                                            else
                                            {
                                                if (sg10)
                                                {
                                                    ...
                                                    timer = true;
                                                    SMIVesselName = "";
                                                    StartCoroutine(SpawnCraftRoutine(OrX_SMI10));
                                                }
                                            }
                                        }
                                    }
