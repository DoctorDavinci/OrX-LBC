       ...
       // [KSPField(isPersistant = true, guiActiveEditor = true, guiActive = true, guiName = "GO WALDO"),
        ...
        IEnumerator WaldoSetup()
        {
            if (!brute && !stayPunkd)
            {
                ...
                hpTracker.maxHitPoints = 3000 + (1000 * level);
        ...
        IEnumerator WaldoUpdateHP()
        {
            ...
            if (hpTracker.Hitpoints <= hpTracker.maxHitPoints / 10)
            {
        ...
        IEnumerator stayPunkdSetup()
        {
            if (!brute && !waldo)
            {
                ...
                hpTracker.maxHitPoints = 2000 + (500 * level);
                hpTracker.Hitpoints += hpTracker.maxHitPoints;
        ...
        IEnumerator BruteSetup()
        {
            if (!stayPunkd && !waldo)
            {
                ...
                hpTracker.maxHitPoints = 1000 + (100 * level);
                hpTracker.Hitpoints += hpTracker.maxHitPoints;
        ...
        private void CheckBomber()
        {
            ...
            if (hpTracker.Hitpoints <= hpTracker.maxHitPoints * 0.8)
            {
                DetonateMineRoutine();
            }
        }
        ...
        IEnumerator UpdateorxRoutine()
        {
            if (!player)
            {
                ...
                kerbal.stumbleThreshold = 50;
                hpTracker = GetHP();
                hpTracker.maxHitPoints = 25;// 50 + (level * 25);
                hpTracker.Hitpoints = hpTracker.maxHitPoints;
                hpTracker.Armor = 1;
        ...
        public void LevelUP()
        {
            if (level != Missions.instance.level)
            {
                ...
                */
                ...
                hpTracker.maxHitPoints = hpMax + 10;
                hpMax = hpTracker.maxHitPoints;
                hpTracker.Hitpoints += 10;
                hp = hpTracker.Hitpoints;
            }
        }
