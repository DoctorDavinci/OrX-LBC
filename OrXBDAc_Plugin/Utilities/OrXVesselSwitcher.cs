...
namespace OrXBDAc
{
    ...
    public class OrXVesselSwitcher : MonoBehaviour
    {
        ...
        public bool missionPaused = true;
        ...
        private List<ModuleOrXBDAc> _orxA;
        private List<ModuleOrXBDAc> _orxB;
        ...
        {
            if (_orxA == null) _orxA = new List<ModuleOrXBDAc>();
            _orxA.Clear();

            if (_orxB == null) _orxB = new List<ModuleOrXBDAc>();
            _orxB.Clear();
            ...
            while (v.MoveNext())
            {
                ...
                var orx = v.Current.FindPartModuleImplementing<ModuleOrXBDAc>();
                if (orx == null)
                {
            ...
            while (kerb.MoveNext())
            {
                ...
                List<ModuleOrXBDAc>.Enumerator orxKerb = kerb.Current.FindPartModulesImplementing<ModuleOrXBDAc>().GetEnumerator();
            ...
            if (missionRunning)
            {
                if (!missionPaused)
                {
                    ...
                    {
                        missionPaused = true;
                        KerbinMissions.instance.LevelUp();
                    }
                }
            }
            else
        ...
        private void OnGUI()
        {
            if (_ready)
            {
                if (_showGui && BDArmorySetup.GAME_UI_ENABLED)
                {
                    ...
                    _windowRect = GUI.Window(396782504, _windowRect, ListWindow, "OrX Vessel Switcher",
        ...
        private void ListWindow(int id)
        {
            if (_orxA.Count >= 0)
            {
                ...
                List<ModuleOrXBDAc>.Enumerator orxA = _orxA.GetEnumerator();
                while (orxA.MoveNext())
                {
                ...
                while (wmb.MoveNext())
                {
                    ...
                    if (GUI.Button(guardButtonRect, "G", guardStyle))
                        ...
                        {
                            var OrXkerbal = wmb.Current.vessel.FindPartModuleImplementing<ModuleOrXBDAc>();
                ...
                List<ModuleOrXBDAc>.Enumerator orxB = _orxB.GetEnumerator();
                while (orxB.MoveNext())
                {
        ...
        private string UpdateKerbalStatus(ModuleOrXBDAc orx, GUIStyle vButtonStyle)
        {
            string status = "";
        ...
        private void SwitchToNextVessel()
        {
            ...
            List<ModuleOrXBDAc>.Enumerator orx = _orxA.GetEnumerator();
            while (orx.MoveNext())
            {
